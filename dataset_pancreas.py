...
def pancreas(overwrite=False, convert_names=True):
    
    tissue = "pancreas"
    base_f = "../data/pancreas/"
    ...
    if not os.path.isfile(u_count_f) or overwrite:
      ...
      idx = (s.obs["nCount_RNA"] < 10**5) & (s.obs["nCount_RNA"] > 10**3.2)
      s = s[idx,:]
      
      # make n_genes between 1200 and 2500
      idx = (s.obs["nFeature_RNA"] > 10**2.6) & (s.obs["nFeature_RNA"] < 10**3.8)
      s = s[idx,:]
      
      idx = s.obs["percent.mt"] < 10
      s = s[idx,:]
